...
TEST(ColorTest, test_color_operations) {
  ...
  EXPECT_FALSE(are_close(color1 + color2, Color{5.f, 16.f, 13.5}));
...
TEST(HdrImageTest, test_basic_HdrImage) {
  ...
  EXPECT_EQ(image1._pixel_offset(9, 5), 9 + 5 * image1.width);
...
TEST(PfmTest, test_pfm_parse_endianness) {
  ...
  EXPECT_THROW(_parse_endianness("0.0"), InvalidPfmFileFormat);
...
TEST(PfmTest, test_pfm_parse_img_size) {
  ...
  // Pass a lambda to prevent potential compiler issues/warnings with the discarded return value in the macro.
