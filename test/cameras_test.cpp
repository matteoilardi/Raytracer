...
TEST(RayTest, test_ray_transformation) {
  ...
  Transformation T = translation{Vec{10.f, 11.f, 12.f}} *
                     rotation_x{0.5f * std::numbers::pi_v<float>}; // Beware that std::sin accepts the angle measured in rads
...
TEST(CameraTest, test_orthogonal_camera_transformation) {
  OrthogonalCamera cam2{1.f, translation{-VEC_Y * 2.f} * rotation_z{0.5f * std::numbers::pi_v<float>}};
...
TEST(CameraTest, test_perspective_camera_transformation) {
  PerspectiveCamera cam2{1.f, 1.f, translation{-VEC_Y * 2.f} * rotation_z{0.5f * std::numbers::pi_v<float>}};
  Ray ray5 = cam2.fire_ray(0.5f, 0.5f);
  PerspectiveCamera cam3{1.f, 1.f, translation{-VEC_Z * 3.f} * rotation_y{0.5f * std::numbers::pi_v<float>}};
...
TEST_F(ImageTracerTest, test_image_orientation) {
  // Fire a ray against top left corner of the screen: it should hit the screen at point (x=0, y=2, z=1).
  // Regression test for issue #4: Fixes the vertical flip caused by the mismatch between v coordinates
  // increasing upwards and HdrImage top-to-bottom row storage.
  Ray top_left_ray = tracer->fire_ray(0, 0, 0.f, 0.f);
  EXPECT_TRUE((Point{0.f, 2.f, 1.f}.is_close(top_left_ray.at(1.f))));

  // Fire a ray against bottom right corner of the screen: it should hit the screen at point (x=0, y=-2, z=1).
