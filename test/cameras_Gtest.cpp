...
TEST(CameraTest, test_ray) {
  ...
  EXPECT_TRUE(ray4.at(0.0).is_close(ray4.origin));
  EXPECT_TRUE(ray4.at(1.0).is_close(Point(5.0, 4.0, 5.0)));
  EXPECT_TRUE(ray4.at(2.0).is_close(Point(9.0, 6.0, 6.0)));
}

// TEST: method Ray::transform
TEST(CameraTest, test_ray_transformation) {
  ...
  EXPECT_TRUE(transformed.direction.is_close(Vec(6.0, -4.0, 5.0)));
}
...
TEST(CameraTest, test_orthogonal_camera) {
  ...
  EXPECT_TRUE(are_close((ray1.direction ^ ray2.direction).squared_norm(), 0.));
  EXPECT_TRUE(are_close((ray1.direction ^ ray3.direction).squared_norm(), 0.));
  EXPECT_TRUE(are_close((ray1.direction ^ ray4.direction).squared_norm(), 0.));
  ...
  EXPECT_TRUE(ray1.at(1.).is_close(Point(0., 2., -1.)));
  EXPECT_TRUE(ray2.at(1.).is_close(Point(0., -2., -1.)));
  EXPECT_TRUE(ray3.at(1.).is_close(Point(0., 2., 1.)));
  EXPECT_TRUE(ray4.at(1.).is_close(Point(0., -2., 1.)));
}
...
TEST(CameraTest, test_orthogonal_camera_transformation) {
  ...
  EXPECT_TRUE(ray5.at(1.0).is_close(Point(0., -2., 0.)));
}
...
TEST(CameraTest, test_perspective_camera_transformation) {
  ...
  EXPECT_TRUE(ray5.at(1.0).is_close(Point(0., -2., 0.)));
  EXPECT_TRUE(ray6.at(1.0).is_close(Point(0., 0., -3.)));
}

// test methods of ImageTracer class
TEST(ImageTracerTest, test_image_tracer) {
  ...
  for (int col = 0; col < tracer.image->width; ++col) {
    for (int row = 0; row < tracer.image->height; ++row) {
      EXPECT_TRUE(are_close(tracer.image->get_pixel(col, row), Color(1., 2., 3.)));
    }
  }
}

// additional test for image tracer vs HdrImage orientation highlighting a bug in original code (issue #4)
TEST(ImageTracerTest, test_image_tracer_coordinate_orientation) {
  std::unique_ptr<HdrImage> img = std::make_unique<HdrImage>(4, 2);
  std::unique_ptr<Camera> cam = std::make_unique<PerspectiveCamera>(1., 2.);
  ImageTracer tracer(std::move(img), std::move(cam));
  ...
  EXPECT_TRUE(Point(0., 2., 1.).is_close(top_left_ray.at(1.)));
  ...
  EXPECT_TRUE(Point(0., -2, -1.).is_close(bottom_right_ray.at(1.)));
}
